 ...
 * 
 * Användning:
 * npx tsx scripts/create-admin.ts admin@cdlp.com "StrongPassword123!"
 */
...
async function createAdminUser(email: string, password: string) {
  try {
    ...
    console.error('❌ Fel vid skapande av admin-användare:', error instanceof Error ? error.message : 'Okänt fel')
    process.exit(1)
  } finally {
...
if (args.length !== 2) {
  ...
  console.log('')
  console.log('Exempel:')
  console.log('  npx tsx scripts/create-admin.ts admin@cdlp.com "MySecurePassword123!"')
  process.exit(1)
}
